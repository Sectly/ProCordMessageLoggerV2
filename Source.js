...
module.exports = class MessageLoggerV2 {
    ...
    if (!global.XenoLib || !global.ZeresPluginLibrary || global.DiscordJS || XenoLibOutdated || ZeresPluginLibraryOutdated) {
      ...
      class k extends BdApi.React.PureComponent {
        constructor(a) {
          super(a), this.state = {
            hasError: !1
          }, this.componentDidCatch = a => (console.error(`Error in ${this.props.label}, screenshot or copy paste the error above to Sectly_playz#1404 for help.`), this.setState({
            hasError: !0
  ...
  initialize() {
    ...
    if (shouldPass(window.Lightcord)) XenoLib.Notifications.warning(`[${this.getName()}] Welcome To Message LoggerV2! (By: Sectly_playz#1404)`, { timeout: 0 });
    let defaultSettings = {
...
*/
    ...
    this.classes = {
      ...
      messages: this.safeGetClass(
        ...
        this.safeGetClass(() => `.${XenoLib.getSingleClass('scroller messages')} > .${XenoLib.getSingleClass('channelTextArea message')}`, 'Dam-youre-a-failure-my-fucking-god'),
        true
      ),
  ...
  openWindow(type) {
    ...
    const loadMessages = () => {
      ...
      try {
        ...
        if (loadAttempts > 4) {
          XenoLib.Notifications.error(`Couldn't load menu messages! Report this issue to Sectly_playz#1404, error info is in console`, { timeout: 0 });
